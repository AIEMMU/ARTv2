...
namespace ArtLibrary.Model.Motion.MotionBackground
{
    internal class MotionBackground : ModelObjectBase, IMotionBackground
    {
        ...
        private bool m_UseGaussianSmoothing;
        public bool UseGaussianSmoothing
        {
            get
            {
                return m_UseGaussianSmoothing;
            }
            set
            {
                if (Equals(m_UseGaussianSmoothing, value))
                {
                    return;
                }

                m_UseGaussianSmoothing = value;
        ...
        public void GenerateMotionBackground(double threshold, int lineThreshold, out Image<Gray, Byte> binaryBackground, out Image<Gray, Byte> extraImage, Rectangle roi, int startFrame = 0)
        {
            ...
            motion.UpdateDetector(Video.FrameCount, lineThreshold, false);

            Video.SetFrame(startFrame);
            for (int i = 0; i < MotionLength; i++)
            {
                //motionImages[i] = Video.GetGrayFrameImage().SmoothGaussian(3).Mat;
            ...
            {
                if (motionImage.CountNonzero()[0] == 0)
                {
                    //No motion
                    extraImage = filteredImage.Clone();
                    binaryBackground = originBinary;
                    return;
                }
                ...
                using (Mat canneyEdges = new Mat())
                {
                    ...
                    //ImageViewer.Show(motionImage);
                    //ImageViewer.Show(filteredImage);
                    extraImage = filteredImage.Clone();
                    //ImageViewer.Show(canneyEdges);
                   ...
                   40); //gap between lines
                    ...
                    using (VectorOfVectorOfPoint apxContour = new VectorOfVectorOfPoint())
                    {
                        ...
                        using (Image<Gray, Byte> test3 = test1.Clone())
                        {
                            ...
                            PointF[] convexHull = new PointF[0];
                            if (finalPoints != null && finalPoints.Length > 0)
                            {
                                convexHull = CvInvoke.ConvexHull(finalPoints, true);
        ...
        public void GenerateMotionBackground(double threshold, int lineThreshold, out Image<Gray, Byte> binaryBackground, Rectangle roi, int startFrame = 0)
        {
            if (Video == null || Video.FrameCount - startFrame <= MotionLength)
            {
                throw new Exception("Video can not be null and motion length must be over " + MotionLength);
